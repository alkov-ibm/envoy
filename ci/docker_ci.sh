...
ENVOY_DOCKER_IMAGE_DIRECTROY="${ENVOY_DOCKER_IMAGE_DIRECTORY:-${BUILD_STAGINGDIRECTORY:-.}/docker-images}"
...
push_images(){
    ...
    if [[ -z "${TYPE}" ]]; then
        ...
        if [[ -z "${SAVE_IMAGES}" ]]; then
            ...
            docker save ${BUILD_TAG} | xz -T0 -1 > "${ENVOY_DOCKER_IMAGE_DIRECTROY}/envoy-docker${TYPE}-${BUILD_TAG}.tar.xz"
        fi
    fi
}
...
if [[ "${AZP_BRANCH}" != "${MASTER_BRANCH}" ]] && \
   ...
   ! [[ "${AZP_BRANCH}" =~ ${RELEASE_TAG_REGEX} ]]; then
    if [[ -z "${BUILD_REASON}" ]]; then
        ...
        echo 'Running CI in non-master branch or tag for docker push, save images instead.'
        SAVE_IMAGES=true
    fi
fi
